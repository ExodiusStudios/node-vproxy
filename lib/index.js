...
var Server = /** @class */ (function () {
    ...
    Server.prototype.listen = function (config, callback) {
        var _this = this;
        var http = new Promise(function (s) { return _this.http.listen(config.port || 80, function () { return s(); }); });
        var http2 = this.https ? new Promise(function (s) { return _this.https.listen(config.sslPort || 80, function () { return s(); }); }) : Promise.resolve();
        if (callback)
            callback();
    ...
    Server.prototype.close = function (callback) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                ...
                if (this.https) {
                    tasks.push(util_1.promisify(this.https.close)());
                }
...
function buildMiddleware(records, proxy, onBuild) {
    ...
    records.forEach(function (record) {
        app.use(vhost_1.vhost(record.match, function (req, res) {
            proxy.web(req, res, __assign({}, record.proxy, { target: record.proxy }));
        }));
    });
    return app;
}
